...
def train(rank, args, sdae_model, shared_model, optimizer, env_config, train_process_finish_flags):
	...
	if(trans_cost_rate is not None and slippage_rate is not None):
		if(args.full_env):
			env = Single_Stock_Full_Env(stock_raw_data, stock_norm_data, starting_capital, min_episode_length, max_episode_length, max_position, trans_cost_rate, slippage_rate)
		else:
		...
		if(args.full_env):
			env = Single_Stock_Full_Env(stock_raw_data, stock_norm_data, starting_capital, min_episode_length, max_episode_length, max_position)
		else:
